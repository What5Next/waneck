...
async function resolveActiveModelName(
  modelParam?: string,
): Promise<string | null> {
...
export async function POST(req: NextRequest) {
  try {
    ...
    const modelName = await resolveActiveModelName(modelParam);
    if (!modelName) {
      return NextResponse.json(
        { error: "사용할 수 없는 모델입니다." },
        { status: 400 },
      );
    }

    // console.log('[/api/chat] model', {
    //   requested: modelParam ?? '(default)',
    //   resolved: modelName,
    // })
