...
export async function GET(
  ...
  try {
    ...
    if (sourceGenres.length > 0) {
      ...
      if (genreError) {
        console.error(
          "[/api/characters/[id]/similar GET] genre pool",
          genreError,
        );
        return NextResponse.json(
          { error: genreError.message },
          { status: 500 },
        );
      }
    ...
    if (candidates.length < limit * 2) {
      ...
      if (popularError) {
        console.error(
          "[/api/characters/[id]/similar GET] popular pool",
          popularError,
        );
        return NextResponse.json(
    ...
    const normalized = ranked.map(normalizeCharacter);
    const commentCounts = await getCommentCountsByCharacterIds(
      normalized.map((character) => character.id),
    );

    return NextResponse.json(withCommentCounts(normalized, commentCounts));
  } catch (err) {
